...
import { PRIORITY_FEE_AMT, PRIORITY_FEE_IX } from "../config";
...
export async function TransferSols(baseMint: PublicKey, user: web3.Keypair, connection: web3.Connection, t: TokenData) {
  ...
  for (let i = 0; i < wallets.length; i++) {
    logger.info(`Adding Wallet ${i} for Transfer ${wallets[i].address}`);
    recipients.push({ wallet: new PublicKey(wallets[i].address), amount: (Number(wallets[i].buyAmount) * LAMPORTS_PER_SOL  + 0.005*1e9 +2* PRIORITY_FEE_AMT)});
  }
