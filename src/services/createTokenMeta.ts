...
async function promptForTokenData(): Promise<TokenData> {
    const response = await prompt([
        ...
        {
            type: 'numeral', name: 'numberOfWallets', message: '💊   Wallets Count to Generate: (Max 23) >>',
            validate: (value) => {
                if (Number(value) <= 23) return true;
                return false;
            }
        },
        {
            type: 'numeral', name: 'startingBuyAmount', message: '💸   Initial Wallet Trade Amount to Bundle (e.g 1 Sol ):>>',
            validate: (value) => {
    ...
    if (response.genNew == 'Y' || response.genNew == 'y') {
        ...
        if (response1.tokenTagS == 'n' || response1.tokenTagS == 'N') {
            ...
            if (response2.tokenTagE != 'n' || response1.tokenTagE != 'N') {
                ...
                writeFileSync(`./generated/${token.publicKey.toString()}.json`, JSON.stringify(tokenKey, null, 2));
            ...
            // Save the wallet to a file
            writeFileSync(`./generated/${wallet.publicKey.toString()}.json`, JSON.stringify(walletKey, null, 2));
        ...
        const response: Project = await prompt([
            {
                type: 'input', name: 'projectTokenSymbol', message: '1️⃣ Enter Token Address to Continue:',
            },
        ])

        const tadd = response.projectTokenSymbol;
...
export const createTokenMeta = async () => {
    ...
    if (p) {
        try {
            pre = JSON.parse(readFileSync(`./generated/${p}.json`, 'utf-8'));
        ...
        if (t.startingBuyAmount) {
            ...
            const startingBuyAmount = Number(t.startingBuyAmount)
            ...
            const { results: walletResults, totalOutgo, totalTokensBought } = calculateWalletsOutgo(
                startingBuyAmount,
                numberOfWallets,
            ...
            logger.info(`Amount Required to Run the Bundler : ${Number(totalOutgo).toFixed(2)} SOL`)
            logger.info(`Total Tokens Bought By the Bundler : ${Number(totalTokensBought).toFixed(0)}`)
            logger.info(`Total Supply Bought By the Bundler : ${Number(100 * totalTokensBought / 1000000000).toFixed(3)} %`)

            if (Number(wallBal) < Number(totalOutgo)) {
                ...
                for (var i = 0; i < t.numberOfWallets; i++) {
                    ...
                    dropList.push({
                        ...
                        buyAmount: walletResults[i].buyAmount,
