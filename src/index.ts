...
import { Keypair, PublicKey, Transaction, SystemProgram, PublicKeyInitData } from "@solana/web3.js";
import { createTokenMeta } from "./services/createTokenMeta";
import { CONFIRMATIONTIMEOUT, connection, EVENT_AUTH, FEERCPT, GLOBALSTATE, mainWallet, PRIORITY_FEE_IX, programID, randomSellWalletsCount, senderWallet, slippageDefault } from "./config";
...
const startMainMenu = async (t: any | {
    ...
    const questions = [{
        ...
        choices: [
            ...
            { name: ' > Exit', message: '6. Exit >', value: 6 },
        ]
    }];
    ...
    if (answers.option == ' > Launcher') {
        const txIds = await LaunchBundler(senderWallet, mainWallet, t.wallets, t.tokenInfo);
    ...
    if (answers.optionstats == 2) {
        ...
        if (response.x == 'x' || response.x == 'X') {
            ...
            const sellAllSwapAndExecute = async () => {
                const qresponse = await axios.get(`https://frontend-api.pump.fun/coins/${t.tokenInfo.tokenAddress}`).then(result => result.data).catch(error => null);

                const bondingCurve = qresponse.bonding_curve
                const globalState = GLOBALSTATE;
                const feeRecipient = FEERCPT
                const bondingCurveAta = qresponse.associated_bonding_curve
                const tradeList: any[] = [];
                ...
                if (tokenAccnt.length > 0 && start > 0) {
